...
- **System Commands**: Run system actions quickly using the `>` prefix:
  - `> lock` — Lock the workstation
  - `> sleep` — Put the system to sleep
  - `> restart` / `> shutdown` — Power options
  - `> settings` / `> control panel` / `> task manager` — Open Windows management tools
